...
class PageListAction extends AbstractAction
{
    ...
    #[GetMapping(path: '/area/pagelist')]
    #[Get(path: '/area/pagelist', summary: '地区列表（分页）', tags: ['后台管理/地区管理'])]
    ...
    #[QueryParameter(name: 'name', description: '地区名称', schema: new Schema(type: 'string'), example: '海南')]
    ...
    public function handle(): ResponseInterface
    {
        ...
        $total = $this->areaDao->total($name);
        if ($total == 0) {
            $this->success(null);
        }

        $data = $this->areaDao->pageList($name, $page, $pageSize);
        return $this->success([
            'list' => $data,
            'total' => $total,
        ]);
